...
from models import Meeting, MeetingAttendance, User, Family, Notification
...
def create():
    ...
    if form.validate_on_submit():
        ...
        family_members = User.objects(family_id=current_user.family_id)
...
@meetings.route('/meetings/<meeting_id>')
@login_required
def view(meeting_id):
    from bson import ObjectId
    meeting = Meeting.objects(id=ObjectId(meeting_id)).first()
    if not meeting or meeting.family_id != current_user.family_id:
        flash('Access denied', 'danger')
        return redirect(url_for('meetings.index'))
    
    attendance = MeetingAttendance.objects(meeting_id=meeting.id)
...
@meetings.route('/meetings/<meeting_id>/edit', methods=['GET', 'POST'])
@login_required
def edit(meeting_id):
    from bson import ObjectId
    if not current_user.is_admin:
...
@meetings.route('/meetings/<meeting_id>/delete')
@login_required
...
@meetings.route('/meetings/<meeting_id>/attendance', methods=['POST'])
@login_required
def update_attendance(meeting_id):
    ...
    attendance = MeetingAttendance.objects(
        meeting_id=meeting.id,
        user_id=user_id
    ).first()
